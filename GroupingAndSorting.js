// You have been provided with an array of product objects. Each product object contains the following properties: productName, category, 
// quantity, price, and rating.
//  Your task is to create a function groupAndSortProducts using only Higher-Order Functions that takes this array of products as input and
//  returns an object where products are grouped by their categories. Additionally, within each category, the products
//  should be sorted in descending order based on their ratings.
...
// Ensure that the function handles the case where there are no products in a partic
...
const products = [
   ...
   let groupAndSortProducts =((ArrOfProducts)=>{
        let obj={}
        ArrOfProducts.map((ele)=>{
           obj[ele.category]= Array(ele)
        })
     return obj
